...
const create = server => {
    ...
    const broadcastOnlineCount = () => {
        ...
        wss.clients.forEach(client => {
            if (onlineCount === wss.clients.size && client.readyState === WebSocket.OPEN) {
                client.send(msg);
            }
        });
    }
    ...
    const addChat = async(ws, msg) => {
        ...
        if (user.level < chatType.minimumLevel || user.points < chatType.minimumEarned) {
    ...
    (async() => {
        ...
        while (true) {
            ...
            const offers = await OfferDone.find({ date: { $gt: prev?.date } }).sort({ _id: -1 });
            ...
            if (offers.length > 0) {
                ...
                wss.clients.forEach(client => {
                    client.send(JSON.stringify({ type: "newOfferDone", newOfferDone: returnAble }));
                });
            }
        }
    })();
