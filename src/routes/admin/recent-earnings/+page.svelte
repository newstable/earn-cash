<script>
    ...
    const getPage = async (page = 1) => {
        const response = await fetch("/api/admin/earningsSearch?page=" + page.toString());
                            ...
                            <tbody>
                                {#each items as item}
                                    <tr>
                                        ...
                                        <td>
                                            <a href="/admin/offer/{item.offerDone.offerId}">{item.offerDone.offerId}</a>
                                        </td>
                                        <td>
                                            {
                                                item.offerDone.wall === 1 ? "Ayet Studios" :
                                                item.offerDone.wall === 2 ? "Lootably" :
                                                item.offerDone.wall === 3 ? "OfferToro" :
                                                item.offerDone.wall === 4 ? "AdGem" :
                                                item.offerDone.wall === 5 ? "Revenue Universe" :
                                                item.offerDone.wall === 6 ? "AdGate" :
                                                item.offerDone.wall === 7 ? "Adscend" :
                                        ...
                                        <td>
                                            {item.offerDone.payout.toLocaleString(undefined, { style: "currency", currency: "USD" })}
                                        </td>
                                        <td>
                                            {date2readable(item.offerDone.date)} ({dateFormatter(item.offerDone.date)})
                                        </td>
                                        <td>
                                            {#if item.offerDone.status == 1}
                                                <span class="custom-tag tag-success tag-transparented">Successful</span>
                                            {:else if item.offerDone.status == 0}
                                                <span class="custom-tag tag-danger tag-transparented">Chargeback</span>
                                            {:else}
                                                <span class="custom-tag tag-info tag-transparented">Other?</span>
                                            {/if}
                                        </td>
                                    </tr>
                                {/each}
