<script>
  ...
  const getPage = async (page = 1) => {
    const response = await fetch(
      "/api/admin/rewards?type=gamble&sent=0&page=" + page.toString()
    );
  ...
  var breadcrumbData = {
    pageTitle: "Gamble requests",
    bcItemActive: "Gamble requests",
  };
</script>
              ...
              <tbody>
                {#each items as item}
                  <tr>
                    ...
                    <td>
                      {item.reward.discord.username}({item.reward.discord.id})
                    </td>
                    <td>
