<script>
    ...
    const getPage = async (page = 1) => {
        const response = await fetch("/api/admin/rewards?type=giftcard&sent=0&page=" + page.toString());
            ...
            <Col lg={12}>
                <Card class="card-default card-md mb-4">
                    <CardBody>
                                    ...
                                    <tr>
                                        <td>
                                            {#key selectedChanges}
                                                ...
                                                <div class="selection" class:selected={selected.includes(item.reward._id)} on:click={() => {
                                                    (selected.includes(item.reward._id)) ? selected.splice(selected.indexOf(item.reward._id), 1) : selected.push(item.reward._id);
                                                    selectedChanges++;
                                                }}/>
                                            {/key}
                                        </td>
                        ...
                        {/key}
                        <div class="btnholder">
                            <Button class="btn-squared radius-xs fs-12 fw-200 text-capitalize" on:click={() => downloadCSV()}>
