...
export const POST = async(request) => {
    ...
    if (!validation.username(data.username)) return response({ success: false }, 406);
    if (!validation.email(data.email)) return response({ success: false }, 406);
    if (!validation.password(data.password)) return response({ success: false }, 406);
    ...
    if (usernameCount > 0) return response({
        success: false,
    ...
    if (emailCount > 0) return response({
        success: false,
        message: "Email taken",
    ...
    var user = new User({
        ...
        picture: "https://eu.ui-avatars.com/api/?background=302f2f&color=ff5a5c&length=1&name=" + data.username
    });
    ...
    if (data.ref !== null) {
        ...
        if (mongoose.Types.ObjectId.isValid(data.ref.toLowerCase().trim())) {
            ...
            referer = await User.findOne({ customReferralCode: data.ref.toLowerCase().trim() });
        }
