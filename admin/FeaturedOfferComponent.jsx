...
import { offerWalls } from "../src/lib/server/offerWalls";
...
const FeaturedOfferComponent = (props) => {
    ...
    return (
        <Wrapper>
            <Input type="text" placeholder="Offer Id" onChange={e => setOfferId(e.target.value)} value={offerId}/>
            <Select onChange={e => setOfferWall(e.target.value)} value={offerWall}>
                {offerWalls.map((offerwall, i) => <option key={offerwall.simple} value={offerwall.wallId}>{offerwall.wallName}</option>)}
            </Select>
            ...
            <SubmitForm variant="contained" size="lg" color="primary" onClick={() => submitForm()}>Add Featured Offer</SubmitForm>
        </Wrapper>
    );
}
